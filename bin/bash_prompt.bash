...
# User color
case $(id -u) in
	0) user_color="$RED"; root="yes" ;;  # root
	*) user_color="\033[38;5;242m" ;;
esac
...
function prompt_command() {
	...
	# Only show username if in ssh or are root
	local user_prompt=
	[ -n "$remote" -o "$root" = "yes" ] && user_prompt="$user_color$USER"

	# Show hostname inside SSH session
	local host_prompt=
	[ -n "$remote" ] && host_prompt="@$(hostname -s)$NOCOLOR"

	# Format prompt
	first_line="$BLUE\w $user_prompt$host_prompt$NOCOLOR$git_prompt$venv_prompt"
	...
	# Terminal title
	local title="$(dirs -0)"
	[ -n "$remote" ] && title="[ssh] ($(hostname -s)) $title"
